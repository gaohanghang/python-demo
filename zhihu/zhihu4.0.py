...
def job1():
    ...
    for table in ret:
        ...
        label = table.xpath(".//div[@class='HotItem-label']/text()")
        if len(label) != 0:
            item['标签'] = label[0]
    ...
    sql = "INSERT INTO hot(hot_item,create_time) VALUES ('" + json.dumps(items,ensure_ascii=False) + "','" + nowTime + "') "
    print(sql)
    try:
        # 执行sql语句
        cursor.execute(sql)
...
# 添加任务并设置触发方式为3s一次
scheduler.add_job(job1, 'interval', seconds=30*60)
